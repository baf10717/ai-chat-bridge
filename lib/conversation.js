(function (root, factory) {
  ...
  function safeFilename(value) {
    const cleaned = normalizeText(value || "conversation")
      .replace(/[\\/:*?"<>|]/g, "-")
      .replace(/\s+/g, "-")
      .replace(/-+/g, "-")
      .replace(/^-|-$/g, "")
  ...
  function createMarkdownFiles(transcript, metadata, maxChars) {
    const limit = Math.max(10000, maxChars || 250000);
    const title = metadata && metadata.title ? normalizeText(metadata.title) : "Conversation";
    const platform = metadata && metadata.platform ? metadata.platform : "ai";
    const filenameBase = `${safeFilename(platform)}-${safeFilename(title)}`;
    const text = String(transcript || "").trim();
    if (!text) throw new Error("沒有可建立 Markdown 的內容");
    if (text.length <= limit) return [{ name: `${filenameBase}.md`, content: text }];
    ...
    for (const section of sections) {
      if (section.length > limit) {
        pushCurrent();
        for (let offset = 0; offset < section.length; offset += limit) {
          chunks.push(section.slice(offset, offset + limit).trim());
        }
        continue;
      }
    ...
    return chunks.map((content, index) => ({
      name: `${filenameBase}-part-${String(index + 1).padStart(2, "0")}-of-${String(total).padStart(2, "0")}.md`,
  ...
  function claudeSnapshotIdFromUrl(url) {
    try {
      const parsed = new URL(url);
      if (parsed.hostname !== "claude.ai") return null;
      const match = parsed.pathname.match(/^\/share\/([0-9a-f-]+)\/?$/i);
  ...
  function claudeBlockToText(value) {
    if (typeof value === "string") return normalizeText(value);
    if (Array.isArray(value)) {
      return value.map(claudeBlockToText).filter(Boolean).join("\n\n");
    }
    if (!value || typeof value !== "object") return "";

    if (typeof value.text === "string" && value.text.trim()) return normalizeText(value.text);
    if (typeof value.thinking === "string" && value.thinking.trim()) return normalizeText(value.thinking);
    if (value.type === "tool_use") {
      const input = value.input && Object.keys(value.input).length
        ? `\n\n\`\`\`json\n${JSON.stringify(value.input, null, 2)}\n\`\`\``
        : "";
      return `[Tool: ${value.name || "unknown"}]${input}`;
    }
    if (value.type === "tool_result") return claudeBlockToText(value.content);
    if (value.content != null) return claudeBlockToText(value.content);
    return "";
  }

  function claudeMessageToText(message) {
    const chunks = [];
    const contentText = claudeBlockToText(message && message.content);
    const topLevelText = normalizeText(message && message.text);
    if (contentText) chunks.push(contentText);
    if (topLevelText && !chunks.includes(topLevelText)) chunks.push(topLevelText);
    return chunks.join("\n\n");
  }

  function extractClaudeSnapshot(payload) {
    if (!payload || typeof payload !== "object") throw new Error("Claude 分享資料格式錯誤");
    const rawMessages = Array.isArray(payload.chat_messages) ? payload.chat_messages : [];
    const messages = rawMessages.map((message) => ({
      role: message && (message.sender === "human" || message.sender === "user") ? "user" : "assistant",
      text: claudeMessageToText(message)
    })).filter((message) => message.text);

    if (!messages.length) throw new Error("Claude 分享資料中沒有可讀取的對話");
    return {
      platform: "claude",
      title: normalizeText(payload.snapshot_name || payload.name) || "Claude shared conversation",
      messages
    };
  }
